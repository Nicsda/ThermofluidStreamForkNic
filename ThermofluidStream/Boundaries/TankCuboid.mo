...
initial equation
  ...
  //check that all inlet and outlet positions are within the tank geometry
   ...
   assert(noEvent(tankCenter[1]-xLength/2<=inletPositions[i,1] and inletPositions[i,1] <= tankCenter[1]+xLength/2), "In \"" + instanceName + "\": Inlet outside tank geometry",level = AssertionLevel.error);
   assert(noEvent(tankCenter[2]-yLength/2<=inletPositions[i,2] and inletPositions[i,2] <= tankCenter[2]+yLength/2),"In \"" + instanceName + "\": Inlet outside tank geometry",level = AssertionLevel.error);
   assert(noEvent(tankCenter[3]-zLength/2<=inletPositions[i,3] and inletPositions[i,3] <= tankCenter[3]+zLength/2),"In \"" + instanceName + "\": Inlet outside tank geometry",level = AssertionLevel.error);
   end for;
   for i in 1:M_outlets loop
   assert(noEvent(tankCenter[1]-xLength/2<=outletPositions[i,1] and outletPositions[i,1] <= tankCenter[1]+xLength/2),"In \"" + instanceName + "\": Outlet outside tank geometry",level = AssertionLevel.error);
   assert(noEvent(tankCenter[2]-yLength/2<=outletPositions[i,2] and outletPositions[i,2] <= tankCenter[2]+yLength/2),"In \"" + instanceName + "\": Outlet outside tank geometry",level = AssertionLevel.error);
   assert(noEvent(tankCenter[3]-zLength/2<=outletPositions[i,3] and outletPositions[i,3] <= tankCenter[3]+zLength/2),"In \"" + instanceName + "\": Outlet outside tank geometry",level = AssertionLevel.error);
   end for;
equation
