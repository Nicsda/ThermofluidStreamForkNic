...
equation
  assert(noEvent(abs(density_derp_h) > 1e-12), "In \"" + instanceName + "\": The volume should not be used with (nearly) incompressible media. Consider using the FlexVolume instead.", dropOfCommons.assertionLevel);
