...
initial equation
  assert(noEvent(pipe1_high > pipe1_low), "In \"" + instanceName + "\": Upper pipe end must be higher then lower end.", AssertionLevel.error);
  assert(noEvent(pipe2_high > pipe2_low), "In \"" + instanceName + "\": Upper pipe end must be higher then lower end.", AssertionLevel.error);

  if init_method == Init.h then
    medium.h = h_0;
  elseif init_method == Init.M then
    x/d_gas+(1-x)/d_liq = V/M_0;
    assert(noEvent(x>=0 and x<=1), "In \"" + instanceName + "\": Initialization by mass might be inaccurate outside the two-phase region", AssertionLevel.warning);
