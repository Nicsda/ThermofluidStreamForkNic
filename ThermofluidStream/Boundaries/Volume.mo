...
equation
  assert(noEvent(abs(density_derp_h) > 1e-12), "In \"" + instanceName + "\": the simple Volume model should not be used with " + "(nearly) incompressible media. Consider using FlexVolume instead.", dropOfCommons.assertionLevel);
