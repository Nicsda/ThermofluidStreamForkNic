...
initial equation
  ...
  elseif initialize_LiquidMass then
    assert(noEvent(V-M_liq_start/liquidDensity > 0),"In \"" + instanceName + "\": Initial liquid mass is larger then the tank can contain",level = AssertionLevel.error);
...
equation
  for i in 1:N_inlets loop
    assert(noEvent(m_flow_in[i] > m_flow_assert), "In \"" + instanceName + "\": Negative massflow at tank inlet", dropOfCommons.assertionLevel);
  end for;
    for i in 1:M_outlets loop
    assert(noEvent(-m_flow_out[i] > m_flow_assert), "In \"" + instanceName + "\": Positive massflow at tank outlet", dropOfCommons.assertionLevel);
  end for;

  assert(noEvent(M > 0), "In \"" + instanceName + "\": Tanks might not become empty");
