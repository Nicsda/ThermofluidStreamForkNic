...
equation
  for i in 1:N loop
   assert(noEvent(m_flow_in[i] > m_flow_assert), "In \"" + instanceName + "\": Negative mass flow rate at volume inlet", dropOfCommons.assertionLevel);
  end for;
  assert(noEvent(-m_flow_out > m_flow_assert), "In \"" + instanceName + "\": Positive mass flow rate at volume outlet", dropOfCommons.assertionLevel);
  assert(noEvent(M > 0), "In \"" + instanceName + "\": Negative mass inside the volume");
