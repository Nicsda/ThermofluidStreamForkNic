...
equation
  ...
  for i in 1:M_outlets loop
    assert(noEvent(-m_flow_out[i] > m_flow_assert), "In \"" + instanceName + "\": Positive mass flow rate at volume outlet", dropOfCommons.assertionLevel);
  end for;
  assert(noEvent(M > 0), "In \"" + instanceName + "\": Negative mass inside volume");
